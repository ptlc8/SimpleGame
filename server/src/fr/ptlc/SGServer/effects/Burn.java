...
public class Burn extends Effect { // copie de Poison
	...
	public Burn(Playable burner, int duration) {
		super("burn", burner, duration);
		targets = new HashSet<Playable>();
		timer.scheduleAtFixedRate(new TimerTask() {
			public void run() {
				for (Playable target : targets)
					target.takeDamage(getSender(), damage, Type.FIRE);
			}
